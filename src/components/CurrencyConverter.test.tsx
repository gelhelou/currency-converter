...
describe("CurrencyConverter", () => {
  ...
  describe("Functional", () => {
    ...
    it("should add a new currency selector when add button is clicked", () => {
      ...
      expect(
        getAllByTestId("currency-converter-select-currenciesTo")
      ).toHaveLength(2);
    });
    ...
    it("should swap from and to currency drop down values when swap button is clicked", () => {
      ...
      const fromPicker = getByTestId(
        "currency-converter-select-currencyFrom"
      ) as HTMLSelectElement;
      const toPicker = getByTestId(
        "currency-converter-select-currenciesTo"
      ) as HTMLSelectElement;
  ...
  describe("Integration", () => {
    ...
    it("should allow multiple conversions at the same time, then wipe result on change", async () => {
      ...
      fireEvent.change(getByTestId("currency-converter-select-currencyFrom"), {
        target: { value: "USD" },
      });
      const addButton = queryByText("Add");
      fireEvent.click(addButton!);
      fireEvent.change(
        getAllByTestId("currency-converter-select-currenciesTo")[0],
        {
          target: { value: "EUR" },
          position: 1,
        }
      ...
      fireEvent.change(
        getAllByTestId("currency-converter-select-currenciesTo")[1],
        {
          target: { value: "GBP" },
          position: 1,
        }
